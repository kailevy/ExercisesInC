...
pthread_t make_thread(void *(*entry)(void *), int *conn)
{
    int ret;
    pthread_t thread;

    ret = pthread_create(&thread, NULL, entry, (void *) conn);
    if (ret != 0) {
...
{
    // Reassigning s causes a segfault which crashes this version of the code. 
    // s = 125;
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        pthread_t tid = make_thread(entry, &connect_d);
