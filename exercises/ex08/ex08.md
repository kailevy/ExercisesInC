...
GLib comes from GTK+ (a gui library) but has non gui functions that are useful. It's portable, so it can be written in different architectures and function the same. The name is very confusing.

*  What are the most important pros and cons of using GLib, as opposed
to another library, or just using core C features?

Pros: Consistency, portability, extra functionality without writing tons of code by hand
Cons: Installation, maybe bloated for a user's need

*  What do you have to do to compile and run a "Hello GLib" example?

Follow these instructions: https://developer.gnome.org/glib/stable/glib-building.html
